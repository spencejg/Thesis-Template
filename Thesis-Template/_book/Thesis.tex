...
\authorshort{S Holmes}
% The month and year that you submit your FINAL draft TO THE LIBRARY (May or December)


\advisor{Advisor A Awesome}
\committee{Pea Nut \& Lee Gume}
\institution{McMaster University}
\degree{Doctor of Philosophy}
\degreeshort{Ph.D.}
\education{M.A.}
\educationlong{M.A. (University of Cool); B.A. (Univeristy of Awesome)}
\month{July}
\year{2000}
\city{Hamilton}
\province{Ontario}
...
\Abbreviations{
This is to list all of the commonly used abbreviations and symbols that
...
\end{itemize}
\section{How to include numbering to figures and
tables}\label{how-to-include-numbering-to-figures-and-tables}

\subsection{Figures}\label{figures}

Starting with numbering for figures that were generated outside of your
thesis markdown file (though this goes against reproduceability) we can
use a latex command \texttt{\textbackslash{}addfigure} which is unique
to this template. There are five arguments as follows:
\texttt{\textbackslash{}addfigure\{\textless{}name\textgreater{}\}\{\textless{}Top\ caption\textgreater{}\}\{\textless{}Bottom\ caption\textgreater{}\}\{\textless{}size\ usually\ use\ 0.8\textgreater{}\}\{\textless{}label\textgreater{}\}}.
The label number will appear at the top and if you do not want a caption
or a label then leave then blank. Keep in mind that you must fill in the
file name, and size so that at a bare minimum the command will be
\texttt{\textbackslash{}addfigure\{filename\}\{\}\{\}\{0.8\}\{\}}. I
have generated a plot showing eruptions and wait times from Old Faithful
and saved it as a .png file in the \emph{figure} directory. I have used
the \texttt{\textbackslash{}addfigure} command to include it
below.\footnote{If you want to see how
  \texttt{\textbackslash{}addfigure} was created visit, but do not
  alter, \emph{Mactemplate.cls}.}

\addfigure{figure/Faithful}{Old Faithful}{Plotting eruptions and waiting for old faithful}{0.8}{faithfig}

So now we can reference our Old Faithful figure as Figure
\ref{faithfig}. Notice that the way in which we have to reference
figures from the \texttt{\textbackslash{}addfigure} command, or more
specifically from any \LaTeX command is
\texttt{\textbackslash{}ref\{\textless{}label\textgreater{}\}} whereas
normally throughout markdown we use
\texttt{\textbackslash{}@ref(fig:\textless{}label\textgreater{})} and
\texttt{\textbackslash{}ref(tab:\textless{}label\textgreater{})} for
figures and tables, respectively.

Now let's do the same thing but considering reproduceability where we
will generate the Old Faithful plot within an R chunk. Notice in the
header of the chunk we first state that we are using R and then we give
this chunk, which is dedicated to generating our plot, a label from
which we can reference the figure later in our document.
\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{Thesis_files/figure-latex/faithfulfig-1} 

}

\caption{Plot for Old Faithful}\label{fig:faithfulfig}
\end{figure}
Now let's reference this figure as Figure \ref{fig:faithfulfig}. Notice
though that you must include a \emph{fig.cap} in the chunk header for it
to be given a table number.

IMPORTANT: When you give a table or a figure a label, it MUST be
alphanumeric with not underscores or spaces!

Clearly the second method of includeing figures is ideal however if you
generate figures in other programs or from data centres where you can
only take an image of the plot then the first where you use
\texttt{\textbackslash{}addfigure} will work well. Just to make things
more confusing, you should have noticed that you reference using
\texttt{\textbackslash{}@ref()} or \texttt{\textbackslash{}ref\{\}}, be
careful.

\subsection{Tables}\label{tables}

Now for a table that is generated using the basic markdown table
generation.
\begin{longtable}[]{@{}ccc@{}}
\caption{\label{tab:inhertable} Correlation of Inheritance Factors for
Parents and Child}\tabularnewline
\toprule
...
\bottomrule
\end{longtable}
So to reference the table we just add Table \ref{tab:inhertable} and
then you can reference tables.

If we want to use the \emph{kable} library we can also easily label and
reference tables.
\begin{longtable}[t]{lr}
\caption[Max Delays by Airline]{\label{tab:flightdelay}Maximum Delays by Airline}\\
\toprule
Airline & Max Arrival Delay\\
\midrule
Alaska Airlines Inc. & 338\\
...
\bottomrule
\end{longtable}
Can we reference this table \ref{tab:flightdelay}. I'm sure many will
use \emph{stargazer} which is a \LaTeX command. Be sure to include the
label option and then you can reference your regression output easily.
